...
RSpec.describe Guard::Asciidoctor do
  ...
  describe '#_output_paths' do
    context 'by default' do
      # it 'should return test/test/test.asciidoctor as [test/index.html]' do
      #   expect(subject.send(:_output_paths, 'test/index.html.Asciidoctor'))
      #     .to eq(['test/index.html'])
      # end

      # it 'should return test/test.sciidoctor as [test/index.htm]' do
      #   expect(subject.send(:_output_paths, 'test/index.htm.Asciidoctor'))
      #     .to eq(['test/index.htm'])
      # end

      # it 'should return test/test.asciidoctor as [test/index.html]' do
      #   expect(subject.send(:_output_paths, 'test/index.Asciidoctor'))
      #     .to eq(['test/index.html'])
      # end
    end
    ...
    context 'when the output option is set to ["demo/output", "demo2/output"]' do
      ...
      # it 'should return test/test.asciidoctor as [demo1/output/test/index.html.Asciidoctor, demo2/output/test/index.html.Asciidoctor]' do
      #   expect(subject.send(:_output_paths, 'test/index.html.Asciidoctor'))
    ...
    context 'when the default extensions is set to "txt"' do
      ...
      # it 'should return test/test.asciidoctor as test/index.txt' do
      #   expect(subject.send(:_output_paths, 'test/index.Asciidoctor'))
      #     .to eq(['test/index.txt'])
      # end

      # it 'should return test/test.asciidoctor as test/index.php due to the second extension' do
    ...
    context 'when the exclude_base_dir option is set to "test/ignore"' do
      ...
      #   expect(subject.send(:_output_paths, 'test/ignore/index.html.Asciidoctor'))
      #     .to eq(['index.html'])
      # end
      ...
      context 'when the output option is set to "demo/output"' do
        ...
        # it 'should return test/ignore/abc/test.asciidoctor as [demo/output/abc/index.html]' do
        #   expect(subject.send(:_output_paths, 'test/ignore/abc/index.html.Asciidoctor'))
  ...
  describe '#run_on_changes' do
    context 'when notifications option set to true' do
      let(:success_message) { "Successfully compiled Asciidoctor to html!\n" }
      ...
      context 'with one output' do
        ...
        it 'should call Notifier.notify with 1 output' do
          message = success_message + '# spec/fixtures/test.asciidoctor -> spec/fixtures/test.html'
          allow(Guard::Compat::UI).to receive(:info)
          expect(notifier).to receive(:notify).with(true, message)
          subject_notifiable.run_on_changes(["#{@fixture_path}/test.html.Asciidoctor"])
        end
      end

      it 'should call Notifier.notify' do
        message = "Successfully compiled Asciidoctor to html!\n"
        message += '# spec/fixtures/test.asciidoctor -> spec/fixtures/test.html'
        expect(notifier).to receive(:notify).with(true, message)
        subject_notifiable.run_on_changes(["#{@fixture_path}/test.asciidoctor"])
      end
      ...
      context 'with two outputs' do
        ...
        it 'should call Notifier.notify with 2 outputs' do
          message = success_message + '# spec/fixtures/test.asciidoctor -> spec/fixtures/test.html, spec/fixtures/test2.html'
          allow(Guard::Compat::UI).to receive(:info)
          expect(notifier).to receive(:notify).with(true, message)
          subject_notifiable.run_on_changes(["#{@fixture_path}/test.asciidoctor"])
        end
      ...
      context 'with helper' do
        ...
        it 'should call Notifier.notify with 1 output' do
          message = success_message + '# spec/fixtures/test3.asciidoctor -> spec/fixtures/test3.html'
          allow(Guard::Compat::UI).to receive(:error)
          allow(Guard::Compat::UI).to receive(:info)
          expect(notifier).to receive(:notify).with(true, message)
          subject_with_helper.run_on_changes(["#{@fixture_path}/test3.asciidoctor"])
  ...
  describe '#compile_Asciidoctor' do
    it 'throws :task_has_failed when an error occurs' do
      ...
      expect { subject.send(:compile_Asciidoctor, "#{@fixture_path}/fail_test.asciidoctor") }
